...
import { BRAND_ANCHORS, SUB_BRANDS } from './deeperThemingRecipeV2';
 ...
 * scoped to `.poc2-suite-*` so they cannot leak into a branded page.
 *
 ...
 *
 * The recipe selects on `[data-theme-poc2][data-brand='x'][data-mode='y']` — a
...
function BrandScope({ brand, mode, children, style }: { brand: BrandKey; mode: Mode; children: ReactNode; style?: CSSProperties }) {
  ...
  return (
    <div data-theme-poc2="" {...attrs} data-mode={mode} style={style}>
      {children}
    </div>
  );
}
 ...
 *  response is an affordance rather than decoration. `--poc2-mark-px` feeds the
...
function Mark({ brand, size = 44 }: { brand: BrandKey; size?: number }) {
  ...
  return (
    <span
      ref={ref}
      className="poc2-mark poc2-mark--live poc2-mark--tilt"
...
export default function SuitePageV2({ mode }: { mode: Mode }) {
  return (
    <div style={{ background: 'var(--background)', color: 'var(--foreground)' }}>
      ...
      >
        <strong style={{ fontSize: 'var(--text-base)', letterSpacing: 'var(--tracking-tight)' }}>
          <span className="poc2-suite-text">dart</span> central
        </strong>
