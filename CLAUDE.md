...
- **Bespoke Storybook Docs template (2026-07-28):** stock autodocs was replaced with our own page, registered globally as `parameters.docs.page` in `.storybook/preview.tsx`. Shape follows the shadcn component-doc reading order — header · Preview/Code · Install · Usage · Composition · Examples · API reference · Accessibility — but every part is built from the library's own components (`Tabs`, `Badge`, `CodeBlock`, `Item`/`ItemGroup`, `Kbd`) and tokens, so the docs page is itself a proof the system composes. Lives in **`.storybook/docs/`**, deliberately outside `src/` — verified `dist/` contains zero Storybook references.
  ...
  - **Code blocks collapse (`.storybook/docs/CodePane.tsx`).** Printing every story's source in full made Button's page 17,300px tall — you scrolled past hundreds of lines to reach the next example. Each Examples snippet now shows ~7 lines (`--h-40`) behind a fade with a **"View code"** button; expanded it caps at `--h-96` and scrolls *inside itself*, so page length never depends on snippet length (Button 17,313 → ~8,800px). The Preview tab's Code panel passes `collapsible={false}` — switching to that tab already *is* the reveal, so it gets the height cap without a toggle. **The wrapper owns the surface:** `docs.scss` sets Storybook's `.docblock-source` to `background: transparent` and paints `--card` on `.ui-docs-code`, which is what lets the fade gradient end on the exact same colour (and puts the code on the same surface as the preview stage). Don't give `.docblock-source` its background back.
