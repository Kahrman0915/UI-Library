...
A React + SCSS component library. 17 shipped components, one shared token file, zero third-party UI libraries. Every visual value comes from `src/styles/tokens.scss`. Every class name follows BEM under a `ui-` prefix.
...
| `Button` | no | 6 variants × 5 styles × 3 sizes + `isLoading` (no brand — `default` IS the theme's primary) |
...
1. `npm run build` — verify the library actually bundles cleanly end-to-end (hasn't been validated yet)
2. Consider a top-level `styles.css` bundle so consumers can import in one line
...
3. Suggested groupings for the `group=` attribute:
   - **Foundations** — Themes, palettes, motion tokens preview
   - **Buttons** — Button, ButtonGroup, CloseButton
   - **Forms** — Input, Textarea, Select, Combobox, Checkbox, RadioGroup, Switch, Field, InputGroup
   - **Feedback** — Alert, Badge, Progress, Spinner
   - **Overlays** — Dialog, Popover, Tooltip, HoverCard, DropdownMenu, ContextMenu
   - **Navigation** — Tabs, Breadcrumb, Item (list rows)
   - **Layout** — Card, Accordion, Collapsible, ScrollArea, Attachment
...
- **Round out the roster:** Slider still missing. (Toast, Skeleton, Command, Separator, Spinner, ModeToggler, Drawer shipped 2026-07 — Drawer fills the "sheet"/edge-panel role; roster table above is due a refresh. Drawer has no Claude Design preview HTML yet.)
...
- **Accessibility audit** — keyboard nav, focus rings, aria-live regions across all 34 components
...
- **Composition rule:** dismissal X = `CloseButton` (Dialog, Alert, Toast all comply); form-label typography = shared `.ui-label__*` classes (Field included); hairline-with-label = `Separator` (FieldSeparator wraps it).
