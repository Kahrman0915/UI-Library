...
- **Figma design-system push — IN PROGRESS (17/57 done, template LOCKED).** Superseded the old "paused mid-Badge" push. One doc page per component in file `jzc2ME8xVmfX1V8OCt2HC2`; Button (`1 · Button`) is the approved reference implementation at template **v4**. Phase 1 complete; Phase 2 at 9/14. **Before touching the Figma file: read `docs/figma-playbook.md` (the recipe) and resume from `docs/figma-ledger.json` (per-component status + all node/variable IDs; next up: Code).** Do not redesign the template without the owner. Key locked patterns: horizontal frames (Overview · Spec · Theming · Examples·Docs·History), owner's side-by-side light/dark spec tables with labeled separators, composition-forward overview (no props table), theme-able fills bound to the Theme collection, the `flag/is-dark` aiden dark-gradient overlay (never aiden-light/dark variants), and the all-zeros design-check lint as ship gate.
