...
| `Chat` (+ `ChatMessageList` `ChatMessage` `ChatBubble` `ChatMessageActions` `ChatMarker` `ChatComposer` `ChatComposerInput` `ChatComposerActions` `ChatComposerSend` `ChatToolCalls` `ChatToolCall` `ChatSuggestions` `ChatSuggestion`) | yes | Aiden AI chat family. Root provides `density` via context; `ChatMessage from` (user/assistant/system) shared to descendants + drives alignment; **Claude-style transcript** — assistant = full-width bubble-less prose, user = contained bubble on the right (uniform corners, no tail), centered `max-w-3xl` column; `ChatBubble` `pending` typing dots + `streaming` blinking caret; `ChatMarker variant` divider/system/status (composes `StatusDot`); composer reuses `.ui-input-wrap` + autosize textarea (Enter sends / Shift+Enter newline), send composes `Button variant="aiden"` (send↔stop on `isStreaming`/`onStop`); `ChatToolCall` collapsible agent-action card (composes `Collapsible`, status running/success/error via `Spinner`/icons, args/result via `Code`); `ChatSuggestion` one-shot prompt pill (momentary button, not `Chip`). Attachments reuse `Attachment`. Content is consumer-provided (no Markdown dep) |
