...
| `Chat` (+ `ChatMessageList` `ChatMessage` `ChatBubble` `ChatMessageActions` `ChatMarker` `ChatComposer` `ChatComposerInput` `ChatComposerActions` `ChatComposerSend`) | yes | Aiden AI chat family (Conversation MVP). Root provides `density` via context; `ChatMessage from` (user/assistant/system) shared to descendants + drives alignment; `ChatBubble` uses `--chat-bubble-*` tokens + `pending` typing dots; `ChatMarker variant` divider/system/status (composes `StatusDot`); composer reuses `.ui-input-wrap` + autosize textarea (Enter sends / Shift+Enter newline), send composes `Button variant="aiden"` (send↔stop on `isStreaming`/`onStop`). Attachments reuse `Attachment`. Content is consumer-provided (no Markdown dep) |
