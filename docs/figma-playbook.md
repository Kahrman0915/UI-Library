...
- **Status:** **59 of 61** components have a v4 doc page **and an index row**; `Mark` (#61) has a **full v4 page but no index row yet**, and `FullScreenDialog` (#60) has a page with no ledger entry beyond a stub. Phases 1–5 shipped by 2026-07-27; `FeaturedIcon` (Phase 3) and `Fab` (Phase 2) followed and are folded into the phase structure. Ledger holds per-component IDs + ~90 lessons.

  > **The index page is the thing that drifts.** It is still titled *"Rebuild Order — 59 components"* and now understates the roster by two. It has drifted once before (reconciled 2026-07-29). Renumbering it is its own pass — don't fold it into a component build, or the component build is what gets blamed when the count is wrong again.
